...
int16_t extensao_sinal(uint16_t val, int bits) {
    uint16_t mascara = 1 << (bits - 1);
    return (val ^ mascara) - mascara;
}
...
void cpu_executar(CPU *cpu, int num_bps, int *breakpoints) {
    uint16_t cond, pc_atual, end, dado;
    int16_t imm10;
    uint32_t res;
    bool saltar;
    int ch;
    ...
    while (1) {
        ...
        switch (opcode) {
            ...
            case 0x1:
                cond = (cpu->ir >> 14) & 0x3;
                imm10 = extensao_sinal((cpu->ir >> 4) & 0x03FF, 10);
                saltar = false;
                ...
                switch (cond) {
                    ...
                    case 1: 
                        ...
                        break;
                    case 2: 
                        saltar = (cpu->flag_z == 0 && cpu->flag_c == 1); 
                        break;
                    case 3: 
                        saltar = (cpu->flag_z == 1 || cpu->flag_c == 0); 
                        break;
                }
            ...
            case 0x5:
                res = (uint32_t)cpu->regs[rm] + (uint32_t)cpu->regs[rn];
            ...
            case 0x6:
                res = (uint32_t)cpu->regs[rm] + imm4_baixo;
                cpu->regs[rd] = (uint16_t)res;
                cpu->flag_z = (cpu->regs[rd] == 0);
                cpu->flag_c = (res > 0xFFFF);
                break;
